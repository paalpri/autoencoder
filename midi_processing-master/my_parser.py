...
class note:
    def __init__(self, _type, _id, _pitch, _pitch_number, _octave, _time, _velocity):
        self.type   = _type
        self.id = _id
        self.pitch  = _pitch
    ...
    for element in track:
        ...
        if type(element) == midi.events.NoteOnEvent:
            ...
            if note_velocity == 0: # this is actually a note being released, so we ignore it
                ...
                track_notes.append( note('PRESS', note_id, pitch_to_note_conv[note_pitch], note_pitch, note_octave, cur_abs_time, note_velocity) )
    ...
    for (track_id, track) in enumerate(pattern):
        if track_id == wanted_track:
            ...
            for i in range(0, len(all_notes)-window, shift):
                print ' '.join([str(aux[all_notes[i+j].pitch_number]) for j in range(window)])
...
def check_tracks(filename, pattern, search_instrument, percentage):
    for (track_id, track) in enumerate(pattern):
        ...
        if(track_instrument_number == search_instrument and float(single)/(single+not_single) >= percentage):
            print "%s %d" % (filename, track_id)
        #else:
        #    print "%s %d %d %d %f" %(filename, track_instrument_number, single, not_single, float(single)/(single+not_single))
    ...
    for (track_id, track) in enumerate(pattern):
        if args.get_midi == track_id:
            ...
            for note in all_notes:
                print(str(aux[note.pitch_number])+' '+str(note.time)+' '+str(note.velocity)+' '+str(note.octave))
